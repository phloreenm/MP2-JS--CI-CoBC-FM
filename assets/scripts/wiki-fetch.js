// Base code SOURCE:  https://femkreations.com/how-to-build-a-wikipedia-search-app-in-9-steps/
...
function fetchWikiResults(x) {
    ...
    // Promise fetching and processing the response from the API:
    fetch(url)
...
// function to print the results on the wikipedia tab:
function printErrorOnPage() {
    fetchedInfoResponse.innerHTML = " ";
    fetchedInfoResponse.insertAdjacentHTML('beforeend', `<h2>Could not retrieve the results for \"${searchingItem}\" </h2>`);
    // myArray.forEach(function(item) {
    //     let itemTitle = item.title;
    //     let itemSnippet = item.snippet;
    //     let itemUrl = encodeURI(`https://en.wikipedia.org/wiki/${item.title}`);
